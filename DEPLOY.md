...
| 情况 | 表现 |
| --- | --- |
| 电脑关机 / 窗口被关 | 请求直接连不上，秒级失败 → 自动降级直连，返回里带 `warning` |
| 令牌填错 / 没填 | 本机端返回 `401 RELAY_UNAUTHORIZED` → 自动降级直连 |
| `RELAY_URL` 还是上次那条旧隧道（域名已换） | 同上，连不上 → 降级 |
| 本机端正常返回业务错误（如 `NO_CAPTIONS`） | **原样透传**，不会再去直连撞一次 |
