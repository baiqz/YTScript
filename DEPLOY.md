...
| `POT_REQUIRED` | 502 | YouTube 对当前出口发起人机校验 | **重试无效**（实测连打 6 次全失败），换一个视频即可。详见 4.5 |
...
实测数据（同一时间、同一云端出口、共 12 个视频，含缓存击穿对照）：
...
**因此在云端要彻底解决，只能换出口**，按性价比排序：

1. **配 `PROXY_URL`**（Vercel → Settings → Environment Variables）指向一个**公网可达**的住宅/小众机房代理。改了之后云端成功率＝本机成功率。注意**不能填** `127.0.0.1:xxxx`，云服务器访问不到你家电脑。
2. **换托管位置**：把同一份代码部署到大厂云之外的小型主机（自有独立 IP，被判风险的概率低得多）。
3. **日常用本地模式**：本机跑 `start.bat` 是 **100% 成功**，云端只当分享入口。
...
处理方式只有两条：换一个视频，或按上面第 1 条配 `PROXY_URL` 换出口。
