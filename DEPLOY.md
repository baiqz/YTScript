...
| | **A. ngrok**（省心） | **B. cloudflared**（不容易坏） |
| --- | --- | --- |
| 要注册吗 | 要（免费，邮箱即可） | **不要** |
| 公网地址 | **永久固定**，配一次再也不管 | **每次重启都会变**，变了就得回 Vercel 改 |
| 免费额度 | 2 万请求/月、1GB 流量 | 无限（无账号隧道无可用性保证） |
| 代价 | 多花 5 分钟注册 | 每次重启 relay.bat 都要回 Vercel 改一次 |
| 国内可达性 | **部分网络需要挂代理才能连上 ngrok** | 实测可直连 Cloudflare 边缘（本项目已验证） |

**怎么选**：看你的代理稳不稳。
你的电脑能直连 ngrok 的边缘服务器 → 选 A，一劳永逸；
连不上（或你的代理经常关）→ 选 B，因为它不依赖代理就能建隧道。
**只是想先看看效果，选 B**（cloudflared.exe 已经放在项目目录里了，现在就能跑）。

> 反直觉但重要：**代理是你这条链路上最容易坏的一环**（它一关，本机取数就全废）。
> 所以「需要代理才能连上的隧道」并不比「每次要改地址的隧道」更省心。
> 两条都装也行 —— `relay.bat` 会自动择优。
