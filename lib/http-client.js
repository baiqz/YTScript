...
async function once(url, opts = {}) {
  ...
  // 探测候选通路时，由调用方显式传入待测通路（__transport / __proxy），
  // 而不是临时改写全局 transport / activeProxy。
  // 原因：候选是「并行」探测的，若各自改写全局再在 finally 里恢复，
  // 恢复的只是各自启动那一刻的快照，最后完成的那个会把全局覆盖成中间值，
  // 于是一条可用通路会被另一条失败候选永久顶掉（且 ensureTransport 见
  // transport 非空就不再重探，故障会一直粘住）。
  const effTransport = opts.__transport || transport;
  const effProxy = opts.__proxy === undefined ? activeProxy : opts.__proxy;

  if (effTransport === 'proxy' && effProxy) {
    const sock = await connectTunnel(effProxy, u.hostname, port, opts.timeout || 12000);
...
async function probeCandidate(cand, timeoutMs) {
  try {
    const r = await once(PROBE_URL, {
      timeout: timeoutMs,
      headers: { 'Accept-Encoding': 'identity' },
      __transport: cand.kind,
      __proxy: cand.kind === 'proxy' ? cand.url : '',
    });
